...
class DatabaseQuery:
    ...
    def _query_custom_sql(self):
        ...
        print("Allowed statements: SELECT, WITH, EXPLAIN, PRAGMA")
        print("Use ';' to end multi-line queries")

        # Initialize variables for multi-line query support
        query_lines = []
        in_multiline = False

        while True:
            # Change prompt based on whether we're in a multi-line query
            ...
            if in_multiline or not line.endswith(";"):
                query_lines.append(line)
                in_multiline = True

                # Check if this line ends the query
                if line.endswith(";"):
                    # Join all lines and process the complete query
                    query = "\n".join(query_lines)
                    query_lines = []
                    in_multiline = False
                else:
            ...
            while normalized_query.startswith("--"):
                ...
                newline_pos = normalized_query.find("\n")
                if newline_pos == -1:
                    # The entire query is a comment
                    normalized_query = ""
                    break
                normalized_query = normalized_query[newline_pos + 1 :].strip()

            # Skip if the entire query was comments
            if not normalized_query:
                continue

            # Check if it's an allowed statement type
            allowed_statements = ["SELECT", "WITH", "EXPLAIN", "PRAGMA"]

            # Convert to uppercase for checking
            upper_query = normalized_query.upper()

            # Check if the query starts with any allowed statement
            is_allowed = any(
                upper_query.startswith(stmt) for stmt in allowed_statements
            )

            if not is_allowed:
                print(
                    f"ERROR: Only {', '.join(allowed_statements)} statements are allowed."
                )
                continue

            # Additional safety check - prevent any data modification keywords
            # even if they appear later in the query
            forbidden_keywords = [
                "INSERT",
                "UPDATE",
                "DELETE",
            ...
            for keyword in forbidden_keywords:
                if keyword in upper_query:
                    print(
                        f"ERROR: Data modification statements are not allowed. Found '{keyword}' in query."
                    )
                    continue
            ...
            try:
                ...
                # For EXPLAIN queries, we might not have results to display
                if upper_query.startswith("EXPLAIN"):
                    # EXPLAIN queries return results that should be displayed
                    self._display_results(cursor, "Query Execution Plan")
                else:
    ...
    def _show_sql_help(self):
        """Show SQL query examples including new allowed statement types."""
        print("\nSQL Query Examples:")
        ...
        print("\n4. Show table structure using PRAGMA:")
        print("   PRAGMA table_info(player_stats);")

        print("\n5. Use WITH clause for complex queries:")
        print("   WITH all_years AS (")
        print("       SELECT DISTINCT year FROM player_stats")
        print("   )")
        print("   SELECT * FROM all_years ORDER BY year;")

        print("\n6. Analyze query performance:")
        print("   EXPLAIN QUERY PLAN")
        print("   SELECT * FROM team_standings WHERE year = 2000;")

        print("\nMulti-line queries:")
        print("   Enter your query across multiple lines.")
        print("   End with a semicolon (;) to execute.")
        print("   Example:")
        print("   SQL> WITH team_summary AS (")
        print("   ...>     SELECT team, AVG(wins) as avg_wins")
        print("   ...>     FROM team_standings")
        print("   ...>     GROUP BY team")
        print("   ...> )")
        print("   ...> SELECT * FROM team_summary")
        print("   ...> WHERE avg_wins > 90;")
