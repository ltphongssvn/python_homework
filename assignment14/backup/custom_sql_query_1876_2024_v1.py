...
class SQLQueryExplorer:
                    ...
                    "b": {
                        ...
                        "sql": """
                            SELECT
                                ...
                                ROUND(MIN(winning_percentage), 3) as worst_team,
                                ROUND(MAX(winning_percentage), 3) as best_team,
                                ROUND(MAX(winning_percentage) - MIN(winning_percentage), 3) as win_pct_spread
                            FROM team_standings
                            WHERE year >= 2000
    ...
    def _custom_query_mode(self):
        ...
        while True:
            ...
            # Safety check
            if not full_query.strip().upper().startswith("SELECT"):
