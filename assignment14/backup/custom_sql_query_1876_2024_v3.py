#!/usr/bin/env python3
# custom_sql_query_1876_2024_v3.py
"""
...
import logging
import math  # Import math at the top level for use in the new class
import sqlite3
import sys
...
# FIX: Added the missing StddevAggregate class definition.
# This class is required for the custom 'STDDEV' aggregate function
# used as a fallback for older versions of SQLite.
class StddevAggregate:
    """
    A custom aggregate class for calculating standard deviation in SQLite.
    ...
    def step(self, value):
        ...
        if value is not None:
            # Values from SQL come as parameters, ensure they are floats for calculation
            self.data.append(float(value))

    def finalize(self):
        """Called at the end. Calculates and returns the standard deviation."""
        # Standard deviation is not meaningful for less than 2 data points
        if len(self.data) < 2:
            return None
...
class SQLQueryExplorer:
    ...
    def __init__(self, db_path: str = "baseball_history.db"):
        ...
        # Enable built-in math functions if available (SQLite 3.35+)
        try:
            self.conn.enable_load_extension(True)
            # Try to load the math extension
            self.conn.execute("SELECT sqrt(4)")
            self.has_math_functions = True
        except sqlite3.Error:  # Catch specific sqlite3.Error
            self.has_math_functions = False
            # Create custom functions for older SQLite versions
            # The 'math' module is now imported globally at the top of the file

            self.conn.create_function("SQRT", 1, math.sqrt)
            self.conn.create_function("POWER", 2, math.pow)
            self.conn.create_function(
                "SQUARE", 1, lambda x: x * x if x else None
            )
            # Custom STDDEV implementation
            ...
            "4": {
                ...
                "queries": {
                    ...
                    "b": {
                        ...
                        "sql": """
                            WITH league_variance AS (
                                SELECT
                                    ts1.year,
                                    ts1.league,
                                    COUNT(DISTINCT ts1.team) as teams,
                                    AVG(ts1.winning_percentage) as mean_win_pct,
                                    MIN(ts1.winning_percentage) as min_win_pct,
                                    MAX(ts1.winning_percentage) as max_win_pct,
                                    -- Calculate variance manually
                                    AVG((ts1.winning_percentage -
                                        (SELECT AVG(ts2.winning_percentage)
                                         FROM team_standings ts2
                                         WHERE ts2.year = ts1.year AND ts2.league = ts1.league)
                                    ) * (ts1.winning_percentage -
                                        (SELECT AVG(ts2.winning_percentage)
                                         FROM team_standings ts2
                                         WHERE ts2.year = ts1.year AND ts2.league = ts1.league)
                                    )) as variance
                                FROM team_standings ts1
                                WHERE ts1.year >= 2000
                                GROUP BY ts1.year, ts1.league
                            )
                            SELECT
                                year,
                                league,
                                ...
                                -- Variance is already calculated, just round it
                                ROUND(variance * 1000, 1) as variance_x1000
                            FROM league_variance
                    ...
                    "a": {
                        ...
                        "sql": """
                            ...
                            stats_with_mean AS (
                                SELECT
                                    *,
                                    (SELECT AVG(batting_avg) FROM batting_stats) as mean_avg
                                FROM batting_stats
                            ),
                            stats_with_variance AS (
                                SELECT
                                    ...
                                    batting_avg,
                                    mean_avg,
                                    AVG((batting_avg - mean_avg) * (batting_avg - mean_avg))
                                        OVER() as variance
                            ...
                            SELECT
                                ...
                                ROUND(mean_avg, 3) as league_avg,
                                -- Z-score without SQRT - using variance comparison instead
                                ROUND((batting_avg - mean_avg) * (batting_avg - mean_avg) / variance, 2) as deviation_score
                            FROM stats_with_variance
                            WHERE (batting_avg - mean_avg) * (batting_avg - mean_avg) / variance > 6.25  -- equivalent to |z| > 2.5
    ...
    def _custom_query_mode(self):
        ...
        print("Note: allow WITH clauses and SELECT.")
